...
/*   Updated: 2022/04/23 22:51:15 by chaejkim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	read_file(t_map *map, int fd, char *read_buf)
{
	int	read_len;

	read_len = read(fd, read_buf, BUFFER_SIZE);
	if (read_len == -1)
		map_error(map, "read error\n");
	read_buf[read_len] = '\0';
	return (read_len);
}
...
static void	lst_to_map(t_map *map)
{
	...
	while (lst)
	{
		...
		{
			free_pptr(map->data);
			map_error(map, "malloc failed (t_map *map.map[row])\n");
		}
...
void	map_parse(t_game *game, char *file_name)
{
	...
	{
		read_len = read_file(game->map, fd, read_buf);
