...
void ClapTrap::beRepaired(unsigned int amount){
	...
	else if (amount >= this->_max_hp || _hp + amount >= this->_max_hp)
		_hp = this->_max_hp;
	else
		_hp += amount;
	_ep--;
...
void	ClapTrap::_setStatus(const ClapTrap &other){
...
void	ClapTrap::_setStatus(const std::string &name){
	this->_name = name;
	this->_hp = this->_max_hp;
	this->_ep = this->_max_ep;
	this->_ad = this->_max_ad;
}