...
static void	buf_to_lst(t_map *map, char *buf, int buf_len)
{
	int	cols;
	t_list	*new_lst;
...
static void	lst_to_map(t_map *map)
{
	...
	while (row <= map->rows)
	{
		//map->map[row] = ft_strndup((char *)lst->content, map->cols);
		//if (map->map[row] == NULL)
		//{
		//	free_pptr(map->map);
		//	read_error("malloc failed (t_map *map.map[row])\n");
		//}
		//map->map[row][map->cols] = 0;
		row++;
...
void	map_parse(t_game *game, char *file_name)
{
	...
	while (read_len == BUFFER_SIZE)
	{
		...
		buf_to_lst(&game->map, read_buf, read_len);
	}
	lst_to_map(&game->map);
